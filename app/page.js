...
const Page = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [mobileRes, fashionRes, electronicRes, homeKitchenRes] =
          await Promise.all([
            axios.get(`${BASE_URL}/mobiles`),
            axios.get(`${BASE_URL}/fashion`),
            axios.get(`${BASE_URL}/electronics`),
            axios.get(`${BASE_URL}/home-kitchen`),
          ]);
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-100 p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 relative">
        <div className="absolute w-full text-center text-5xl mt-4">
          <h1> Explore Mobile</h1>
        </div>
      ...
      <div className="min-h-screen mt-4">
        <div className=" w-full text-center text-5xl flex items-center justify-center">
